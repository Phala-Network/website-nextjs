...
const ContactUsSchema = S.Struct({
  name: S.String,
  message: S.String,
...
export async function POST(req: Request) {
  ...
  const exit = await Effect.runPromiseExit(Effect.gen(function* (_) {
    if (!process.env.MAILERLITE_GROUP_CONTACT_US) {
      return yield* _(Effect.die('MAILERLITE_GROUP_CONTACT_US is not set.'))
    }
    const raw = yield* _(Effect.tryPromise(() => req.text()))
    const parsed = yield* _(safeParseJson(raw))
    const decoded = yield* _(S.decodeUnknown(ContactUsSchema)(parsed))
    const result =yield* _(upsertSubscriber({
      ...decoded,
      groups: [process.env.MAILERLITE_GROUP_CONTACT_US]
    }))
