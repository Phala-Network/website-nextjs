...
export function SimpleCard({ label, title, className, maxTitleLines, children }: SimpleCardProps) {
  const theme = useSectionTheme()
  return (
    <div
      className={cn(
        "flex flex-col gap-2.4 lg:gap-4 p-6 lg:p-8 border rounded",
        theme === 'light' ? 'border-blackAlpha-200 bg-white' : 'border-whiteAlpha-200 bg-whiteAlpha-50',
        className,
      )}
    >
      <dt>
        ...
        <h4
          className={cn(
            "text-20 lg:text-24 font-bold tracking-tight text-balance",
            label && 'mt-4',
            maxTitleLines && `h-[${maxTitleLines}lh] max-h-[${maxTitleLines}lh] overflow-hidden`,
