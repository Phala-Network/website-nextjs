...
import { atom, useSetAtom, useAtomValue, type WritableAtom } from 'jotai'
...
export const inMemoryCurrentAccountAtom =
  atom<InjectedAccountWithMetaAndName | null>(null)
...
const sortedAccounts = (acc: Readonly<InjectedAccountWithMetaAndName[]>) => {
  return sort(
    (a, b) =>
      (propOr('', 'name', a) as string).localeCompare(propOr('', 'name', b)),
    acc
  )
}
...
export const walletSelectModalVisibleAtom = atom(
  false
) as unknown as WritableAtom<boolean, [boolean], boolean>

export const accountSelectModalVisibleAtom = atom(
  false
) as unknown as WritableAtom<boolean, [boolean], boolean>

export const useRestoreLastSelectedAccount = () => {
  useAtomValue(lastSelectedWeb3ProviderAtom)
